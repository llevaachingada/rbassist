...
class SectionRerankTests(unittest.TestCase):
    ...
    def test_recommend_without_flag_unchanged_order(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                ...
                mock.patch.object(recommend, "load_section_embeddings") as load_section_embeddings,
        ...
        load_section_embeddings.assert_not_called()
    ...
    def test_playlist_transition_diagnostics_report_section_coverage(self) -> None:
        ...
        self.assertEqual(result.diagnostics["section_scores_seed_ready"], True)
        ...
        self.assertEqual(result.diagnostics["section_scores_applied"], True)
        self.assertEqual(result.diagnostics["transition_candidate_score_count"], 1)
        self.assertEqual(result.diagnostics["selected_section_scores_applied"], True)
    ...
    def test_playlist_transition_diagnostics_distinguish_requested_from_applied(self) -> None:
        seed_paths = [f"seed_{idx}" for idx in range(3)]
        candidate_path = "candidate"
        meta = {"tracks": {}}
        vectors = {}
        ...
        self.assertEqual(result.diagnostics["section_scores_requested"], True)
        self.assertEqual(result.diagnostics["section_scores_seed_ready"], False)
        self.assertEqual(result.diagnostics["seed_section_late_count"], 0)
        self.assertEqual(result.diagnostics["candidate_section_intro_count"], 0)
        self.assertEqual(result.diagnostics["section_scores_applied"], False)
        self.assertEqual(result.diagnostics["transition_candidate_score_count"], 0)
        self.assertIsNone(result.diagnostics["transition_candidate_score_mean"])
        self.assertEqual(result.diagnostics["selected_section_scores_applied"], False)
        self.assertEqual(result.diagnostics["selected_transition_score_count"], 0)
        self.assertIsNone(result.diagnostics["selected_transition_score_mean"])
        self.assertNotIn("transition_score", result.added_tracks[0].component_scores)
