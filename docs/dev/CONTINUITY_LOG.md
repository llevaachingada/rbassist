...
### 2026-04-15
- Goal: Integrate the next safe advanced-matching slice through learned-similarity training and comparison without rewriting the primary recommendation system.
- Changes made: added `rbassist/similarity_head.py` with the pairwise MLP from the research plan, CUDA-preferred device selection, checkpoint loading, and score inference; added `scripts/train_similarity_head.py` to train from playlist-pair JSONL with AdamW, validation split, early stopping, and CUDA default with CPU fallback; added opt-in `rbassist recommend --learned-similarity --w-learned-sim` wiring; added benchmark rows `G` (`C + harmonic scoring`) and `H` (`C + learned similarity`) for listening-review comparisons. The playlist-pair loader was hardened to skip smart playlists and individual load failures during discovery.
- Evidence / outputs: this is a repo-only integration slice. No live `data/meta.json`, embeddings, indexes, checkpoints, training datasets, model files, or Rekordbox state were mutated.
- Validation: `python -m pytest tests/test_similarity_head.py tests/test_recommend_index.py tests/test_playlist_pairs.py tests/test_benchmark_embeddings.py` passed (`21 passed`); `python -m compileall rbassist scripts tests\test_similarity_head.py` passed; CLI help passed for `rbassist recommend`, `scripts\train_similarity_head.py`, and `scripts\benchmark_embeddings.py`.
- Current blockers or risks: no real playlist-pair dataset has been exported or inspected yet, and no production similarity head has been trained. CUDA is now the default preference for the new trainer/inference path, but real CUDA throughput still needs to be confirmed on the local machine with an actual dataset.
- Next recommended step: dry-run playlist-pair export, write `data\training\playlist_pairs.jsonl` only if counts look sane, train `data\models\similarity_head.pt` on CUDA, then run benchmark rows `C,D,G,H` with fixed golden seeds for listening review.

### 2026-04-15
- Goal: Execute the approved six-step advanced-matching rollout locally with CUDA as the default model device and keep all live data changes backup-first.
- Changes made: backed up live metadata to `data/backups/meta_before_20260415_embedding_steps_1_6.json`; reran the quarantined `AITCH - Wait.mp3` section diagnostic on CUDA with profiling enabled, which completed successfully and wrote the missing intro/core/late section sidecars; backfilled opt-in harmonic chroma/tonnetz profiles for the reviewed `C:\Users\hunte\Music\BREAKS\2024 july august` crate; exported a real Rekordbox playlist-pair dataset; trained the first learned similarity head on CUDA; added optional benchmark listening-review JSON output with overlap reporting against baseline row `C`.
- Evidence / outputs: AITCH diagnostic used `device=cuda`, decoded `9,525,359` samples, trimmed to `5,292,000`, used `mert_flattened_item_count=3`, `actual_mert_batch_size=3`, `load_audio_s=3.679`, and `mert_encode_s=5.506`; the crate harmonic-profile run analyzed `495` files and all `495` scoped metadata rows now have harmonic profiles; playlist-pair export wrote `116,652` rows (`38,884` positives, `77,768` negatives) from `267` usable playlists with `19` skipped; `scripts/train_similarity_head.py` trained `data\models\similarity_head.pt` on `cuda` using `50,000` rows and reported best validation loss `0.6558820068836212`; benchmark rows `C,D,G,H` wrote `reports/benchmark_learned_similarity_20260415.json` and `reports/benchmark_learned_similarity_20260415_listening_review.json`.
- Validation: `python -m pytest tests/test_playlist_pairs.py` passed (`7 passed`) after live DB discovery exposed smart-playlist/load-error handling; `python -m pytest tests/test_benchmark_embeddings.py` passed (`11 passed`) after adding listening-review overlap and CLI-write coverage; final focused suite `python -m pytest tests/test_similarity_head.py tests/test_recommend_index.py tests/test_playlist_pairs.py tests/test_benchmark_embeddings.py tests/test_harmonic_compatibility.py tests/test_playlist_expand.py` passed (`46 passed`); `python -m compileall rbassist\playlist_pairs.py tests\test_playlist_pairs.py`, `python -m compileall scripts\benchmark_embeddings.py tests\test_benchmark_embeddings.py`, and `python -m compileall rbassist scripts tests\test_similarity_head.py tests\test_playlist_pairs.py tests\test_benchmark_embeddings.py` passed.
- Current blockers or risks: the first learned-similarity smoke is operational but not quality-proven; rows `G` and `H` produced the same top-list overlap as baseline `C` for the current three-seed smoke, so the model should remain opt-in until broader seeds and manual listening review show a real ranking improvement. Generated live artifacts under `data\training`, `data\models`, `data\runlogs`, and the updated `data/meta.json` remain local operator state rather than committed repo fixtures.
- Next recommended step: broaden the benchmark seed set and candidate pool, review the listening-review JSON by ear, then tune `--w-learned-sim` or retrain with more rows only if the broader comparison shows useful movement.

### 2026-04-15
- Goal: Harden the advanced-matching GUI integration so the NiceGUI surfaces expose the new opt-in signals instead of leaving them CLI-only.
- Changes made: Discover filters now include cached profile harmony and learned-fit sliders plus an explicit learned playlist-pair model checkbox; the GUI-neutral Discover service can score cached chroma/tonnetz harmony and the trained similarity head, using `cuda` as the default learned-model device. Discover rows and detail text now surface profile-harmony and learned-fit scores. Crate Expander advanced controls now expose existing backend flags for profile harmony (`use_harmonic_key_scores`) and section flow (`use_section_scores`) without changing preset defaults.
- Evidence / outputs: this was a repo-only GUI/hardening slice; it did not mutate `data/meta.json`, training datasets, model files, indexes, checkpoints, audio files, or Rekordbox state.
- Validation: `python -m pytest tests/test_ui_services.py tests/test_ui_discover.py tests/test_ui_crate_expander.py tests/test_ui_components.py tests/test_ui_state.py` passed (`27 passed`); broader focused suite `python -m pytest tests/test_similarity_head.py tests/test_recommend_index.py tests/test_harmonic_compatibility.py tests/test_playlist_expand.py tests/test_section_rerank.py tests/test_benchmark_embeddings.py tests/test_ui_services.py tests/test_ui_discover.py tests/test_ui_crate_expander.py tests/test_ui_components.py tests/test_ui_state.py tests/test_bridge_boundaries.py` passed (`75 passed`); `python -m compileall rbassist scripts tests\test_ui_services.py tests\test_ui_crate_expander.py tests\test_ui_discover.py` passed.
- Current blockers or risks: learned-fit remains opt-in because the first trained head is operational but not quality-proven; Settings still does not provide a dedicated model-path picker, so Discover uses the default `data/models/similarity_head.pt` path unless filters are extended later.
- Next recommended step: run a live NiceGUI smoke: open Discover, enable learned fit/profile harmony on a known seed, compare rankings against CLI `rbassist recommend`, then generate one Crate Expander run with Profile harmony and Section flow enabled and inspect the detail components.
